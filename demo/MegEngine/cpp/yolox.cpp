...
static void generate_yolox_proposals(std::vector<GridAndStride> grid_strides,
  ...
  const int num_class =
      80; // COCO has 80 classes. Modify this value on your own dataset.
  ...
  try {
    if (argc != 4) {
      std::cout << "Usage : " << argv[0]
                << " <path_to_model> <path_to_image> <device_name>"
                << std::endl;
      return EXIT_FAILURE;
    }
    ...
    if (device_name == "cuda") {
    ...
    } else if (device_name == "multithread") {
      load_config.comp_node_mapper = [](CompNode::Locator &loc) {
      ...
      std::cout << "device_name only support cuda or cpu or multithread"
                << std::endl;
      return EXIT_FAILURE;
    }
